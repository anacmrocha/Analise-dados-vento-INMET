...
def exibir_micro_anual(dados_positivos):
    ...
    st.subheader("⚠️ Para opção BoxPlot, escolha uma única variável.")
...
def rajada_direcao(dados_positivos):
    ...
    if not df_foco.empty:
        limite = df_foco['Rajada_Máxima'].quantile(0.90)
        ...
        plt.colorbar(sc, label='Intensidade da Rajada (m/s)', pad=0.1, shrink=0.6)
        plt.title(f'Direção das Rajadas Extremas (P90+) - {estacao}: {titulo_temp}', pad=30)
        st.pyplot(fig, use_container_width=True)
    else:
